...
FI_PATH = ROOT / "data/processed/fi/aggregate/reconstructed.jsonl"
PRICE_DIR = ROOT / "data/raw/prices"
OUTPUT_DIR = ROOT / "data/processed/analysis"
OUTPUT_PATH = OUTPUT_DIR / "fi_price_features.jsonl"
METADATA_PATH = OUTPUT_DIR / "fi_price_features_metadata.json"
...
def add_fi_features(
    ...
    frame["previous_max_individual_position_pct"] = (
        grouped[
            "max_individual_position_pct"
        ].shift(1)
    )
    frame["previous_max_position_share_pct"] = (
        grouped[
            "max_position_share_pct"
        ].shift(1)
    )
    ...
    frame["short_interest_relative_change"] = np.where(
        valid_relative,
        frame["short_interest_delta_pp"]
        / frame["previous_short_interest_pct"],
        np.nan,
...
def main() -> None:
    ...
    metadata = {
        ...
        "entry_price_rule": (
            "first available "
            "trading-day close on or "
            "after FI snapshot date"
        ),
        "forward_return_rule": (
        ...
        "min_return_5d_rule": (
            "minimum close-to-entry "
            "return across trading "
            "days 1 through 5"
        ),
        "max_return_5d_rule": (
            "maximum close-to-entry "
            "return across trading "
            "days 1 through 5"
        ),
        "identity_rule": (
