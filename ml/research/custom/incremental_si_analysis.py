...
OUTPUT_DIR = Path(
    "data/processed/ml/research/custom/"
    "incremental_si"
)
...
def cross_sectional_group(
    ...
    """
    Assign cross-sectional quantile groups
    independently for every snapshot date.

    Group 0 = lowest.
    Group groups-1 = highest.
    """
    result = pd.Series(
        np.nan,
        index=frame.index,
...
def conditional_si_decile(
    ...
    """
    Volatility tercile per date.

    SI decile within each
    date + volatility tercile.
    """
...
def main() -> None:
    ...
    metadata = {
        ...
        "feature_rows":
            int(len(document["decile_analysis"])),
