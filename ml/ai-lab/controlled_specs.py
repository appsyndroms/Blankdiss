...
def discover_controlled_specs() -> list[ResearchSpec]:
    ...
    from autonomous AI Lab execution. They must be executed
    ...
    for path in sorted(
        ...
        #
        # A locked prospective confirmation test must never become
        # an autonomous AI Lab execution candidate.
        #
        # The confirmation may still be executed explicitly through
        # the normal research runner.
        # ----------------------------------------------------------
        if (
            metadata.get("locked") is True
            and str(
                metadata.get(
                    "stage",
                    "",
