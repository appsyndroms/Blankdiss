...
V0.1:
- grupperar efter förändring i procentenheter
- median
- medel
- andel positiva
- antal observationer
...
BINS = [
    (-float("inf"), 0.0, "Minskning"),
    (0.0, 0.25, "0–0,25 pp"),
    (0.25, 0.50, "0,25–0,50 pp"),
    (0.50, 1.00, "0,50–1,00 pp"),
    (1.00, float("inf"), ">1,00 pp"),
]
    ...
    for label, group in events.groupby(
        ...
        for days in (1, 5, 20, 60):
            ...
            if column not in group:
                row[f"median_{days}d"] = None
                row[f"mean_{days}d"] = None
                row[f"positive_{days}d"] = None
                continue
...
def write_analysis(
    ...
    path = (
        ANALYSIS_DIR
        / f"analysis_{date.today().isoformat()}.json"
    )
...
def main() -> None:
    ...
    print(
        f"Analys: {len(results)} grupper → {path}"
    )
